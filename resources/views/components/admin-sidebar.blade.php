...
<aside id="sidebar"
    ...
    <div class="flex items-center gap-4 h-20 px-6 bg-emerald-900 border-b border-yellow-500 flex-shrink-0 overflow-hidden relative">
        {{-- Subtle Batik Accent --}}
    ...
    <nav class="flex-1 px-4 py-8 space-y-8 overflow-y-auto sidebar-scroll">
        @foreach ($adminNavGroups as $group)
            <div>
                <p class="px-4 mb-4 text-[10px] font-black text-emerald-500/50 uppercase tracking-[0.25em]">
                ...
                <div class="space-y-1">
                    @foreach ($group['items'] ?? [] as $item)
                        ...
                        <a href="{{ $href }}"
                            ...
                            class="group flex items-center gap-3 py-3 rounded-lg text-sm font-bold transition-all duration-300 {{ $itemClasses }} {{ $missing ? 'opacity-40 pointer-events-none' : '' }}">
                            ...
                            <span class="whitespace-nowrap tracking-tight">{{ $item['label'] ?? $item['id'] }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
    ...
    <div class="p-6 border-t border-white/5 bg-emerald-900/30">
        <div class="flex items-center gap-4">
            <div class="relative">
                <img class="size-10 rounded-full border-2 border-yellow-500/30"
            ...
            <a href="/login" class="ml-auto text-emerald-500 hover:text-red-400 transition-all hover:scale-110"
                title="Keluar">
