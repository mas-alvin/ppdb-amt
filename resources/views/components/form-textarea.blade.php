...
<div class="space-y-2">
    <div class="relative pt-2 logic-floating-wrapper">
        ...
        <textarea id="{{ $id ?? $name }}" name="{{ $name }}" rows="{{ $rows }}"
            ...
            @class([
                'peer floating-label-input w-full py-3.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-emerald-700 focus:ring-2 focus:ring-emerald-700/20 rounded-xl text-sm transition-all shadow-sm focus:outline-none text-gray-800 dark:text-gray-100 z-10',
                'has-icon' => $icon,
...
<style>
    ...
    /* Vertical icon alignment for textarea */
